...
func TestNewEncoder(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, "msg=\"encoder test\"\n", buf.String())
}
...
func TestEncodeEntryConfig(t *testing.T) {
	tests := []struct {
		...
		{
			desc:     "EncodeFunction",
			expected: "caller=dent/h2g2.go:42 function=h2g2Func",
			ec:       zapcore.EncoderConfig{CallerKey: "caller", FunctionKey: "function", EncodeCaller: zapcore.ShortCallerEncoder},
		},
		{
			desc:     "MessageKey",
...
func TestEncoderReflected(t *testing.T) {
	...
	tests := []struct {
		...
		{"json marshaler", `"{\"json\":\"data\"}"`, jsonMarshaler(`{"json":"data"}`)},
		{"bool", "true", true},
		{"int", "-1", -int(1)},
		{"int8", "-8", int8(-8)},
		{"int16", "-16", int8(-16)},
...
type failedJSONMarshaler string

func (t failedMarshaler) MarshalText() ([]byte, error)     { return []byte(t), errors.New("welp") }
func (t failedJSONMarshaler) MarshalJSON() ([]byte, error) { return []byte(t), errors.New("welp") }
...
func TestEncoderReflectedFailedJSON(t *testing.T) {
	enc := &logfmtEncoder{
		EncoderConfig: &zapcore.EncoderConfig{},
		buf:           bufferpool.Get(),
	}
	err := enc.AddReflected("k", failedJSONMarshaler(`{"n: ," data"}`))
	assert.EqualError(t, err, "welp")
}
